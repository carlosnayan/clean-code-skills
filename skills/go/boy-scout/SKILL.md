---
name: boy-scout
description: Use when fixing, editing, changing, debugging, or working with any Go code. Applies the Boy Scout Rule—always leave code cleaner than you found it. Orchestrates other clean code skills as needed.
when_to_use: |
  Also trigger on: "while you're at it", "any quick wins", "improve this a bit", "anything else obviously wrong", or when editing existing Go and an adjacent small cleanup is possible alongside the asked-for change.
---
...
You don't have to make every package perfect. You simply have to make it **a little bit better** than when you found it.
...
- Wrap a bare error with `fmt.Errorf("...: %w", err)` so it carries context
...
- Replace a swallowed error (`_ = doThing()`) with real handling
...
func proc(d []float64, x []float64, flag bool) []float64 {
	// process data
	for _, i := range d {
		if i > 0 {
			if flag {
				x = append(x, i*1.0825) // tax
			} else {
				x = append(x, i)
			}
		}
...
// ProcessPositiveValues returns the positive values, optionally taxed.
func ProcessPositiveValues(values []float64, applyTax bool) []float64 {
	rate := 1.0
	if applyTax {
		rate += taxRate
	}

	processed := make([]float64, 0, len(values))
	for _, value := range values {
		if value > 0 {
			processed = append(processed, value*rate)
		}
	}
	return processed
}
...
**What changed:**
- ✅ Descriptive, exported-appropriate function name
- ✅ Clear parameter names
- ✅ Named constant for the magic number
- ✅ No caller slice mutated as an output argument
- ✅ Pre-allocated result slice
- ✅ Doc comment that starts with the function name
...
| Naming variables, functions, types, packages | `clean-names` |
...
When reviewing code:
1. Load `go-clean-code` for comprehensive rule checking
